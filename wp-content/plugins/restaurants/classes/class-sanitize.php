...
class Restaurants_Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'restaurants' ) );
		 ...
		 */
		do_action( 'restaurants_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'restaurants_post_sanitize', $sanitized );
